...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--coverage', type=str, metavar="COVERAGES", default="0.2",
        help=('Coverage to start subsample at. (Default 0.01x')
    )
    ...
    parser.add_argument('--threshold', metavar="FLOAT", type=float,
                        default=0.95,
                        help='Percent of successful tests (Default 0.95)')
    ...
    tests = []
    decimal = (
        1.0 / 10**abs(Decimal(args.coverage.rstrip("0")).as_tuple().exponent)
    )
    places = abs(Decimal(str(decimal).rstrip("0")).as_tuple().exponent)
    fails = 0
    while 1:
        reads = int(GENOME_SIZE * float(coverage) / LENGTH)
        print("Working on coverage: {0:f}x ({1} reads)".format(coverage, reads))
        MATCHES = 0
        ...
        MATCHES = len(glob.glob("{0}/*lef.txt.gz".format(path)))
        print("\tTests: {0}, Successes: {1}. ({2} {3:f})".format(
            args.replicates, MATCHES, float(MATCHES/args.replicates), places
        ))

        status = "SUCCESS"
        if float(MATCHES/args.replicates) >= args.threshold:
            if not(coverage - decimal):
                decimal = decimal / 10  # shift decimal place right
                places += 1
            coverage = coverage - decimal
        else:
            if coverage == args.coverage:
                print("Select a larger starting coverage.")
            # Return to previous coverage and shift the decimal place
            coverage = coverage + decimal
            decimal = decimal / 10
            places += 1
            coverage = coverage - decimal
            status = "FAILED"
        tests.append([
            "{0:.6f}".format(old_coverage), str(args.replicates), str(MATCHES),
            status
        ])
        old_coverage = coverage
        if places > 4:
    ...
    with open('{0}-summary.txt'.format(args.sample), 'w') as output_handle:
        output_handle.write("coverage\ttests\tsuccesses\tstatus\n")
        for row in tests:
